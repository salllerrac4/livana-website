...
const benefits = ['100% tinh dầu nguyên chất', 'Giúp ngủ sâu, giảm căng thẳng', 'Mùi hương tinh tế, dịu nhẹ'];

const HeroSection = () => {
  return (
    <section className="hero-section relative w-full overflow-hidden font-display">
      <div className="hero-content mx-auto grid w-full max-w-screen-xl gap-6 px-4 pb-6 pt-5 sm:px-6 md:gap-8 md:pb-10 md:pt-9 lg:grid-cols-2 lg:gap-12 lg:px-10 lg:pb-12 lg:pt-11 xl:gap-16">
        <div className="order-2 flex w-full flex-col items-start space-y-5 -ml-1 pl-1 text-left sm:ml-0 sm:space-y-6 sm:pl-0 lg:order-1 lg:-ml-1 lg:items-start lg:space-y-7 lg:pl-1 lg:text-left">
          <div className="hero-reveal hero-reveal--1 flex flex-wrap items-center gap-3">
            ...
            <div className="hero-proof">
              ...
              <span className="text-xs font-semibold uppercase tracking-[0.18em] text-textMain/70">4,9/5 từ khách hàng</span>
            </div>
          </div>
          <h1 className="hero-headline hero-display hero-reveal hero-reveal--2 max-w-2xl self-start pl-1 text-[clamp(2.1rem,5.4vw+0.35rem,3.9rem)] font-semibold leading-[1.02] tracking-tight text-textMain sm:max-w-3xl sm:pl-0 md:leading-[1.04]">
          ...
          <p className="hero-subtitle hero-reveal hero-reveal--3 max-w-xl self-start pl-1 text-base text-textMain/75 sm:pl-0 md:text-lg">
            Chắt lọc từ thảo mộc nguyên chất, thiết kế tối giản và lan tỏa dịu nhẹ cho phòng ngủ, góc làm việc và mọi khoảnh khắc thư giãn.
          </p>
          <ul className="hero-reveal hero-reveal--4 max-w-2xl space-y-3 self-start pl-1 text-base text-textMain/75 sm:pl-0 md:text-lg">
            {benefits.map((text) => (
              <li key={text} className="flex items-start gap-3">
                <span className="mt-[2px] inline-flex h-6 w-6 items-center justify-center rounded-full border border-primary/30 bg-primary/5 text-primary" aria-hidden="true">
          ...
          <p className="hero-note hero-reveal hero-reveal--6 pl-1 text-sm text-textMain/65 sm:pl-0">Tư vấn chọn mùi miễn phí | Giao hàng nhanh toàn quốc</p>
        </div>
      ...
      <div className="text-textMain lg:border-t lg:border-primary/10 -mb-4 sm:mb-0">
        <div className="mx-auto grid w-full max-w-screen-xl grid-cols-2 sm:grid-cols-2 lg:grid-cols-[1fr_1fr_1fr_1.45fr]">
          {metrics.map((item, idx) => (
            <div key={item.label} className={`px-7 py-5 ${idx > 0 ? 'lg:border-l lg:border-primary/20' : ''}`}>
