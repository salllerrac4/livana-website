...
const socialLinks = [
  {
    name: 'Facebook',
    href: brandProfiles.find((item) => item.name === 'Facebook')?.url ?? 'https://www.facebook.com/livana.official',
    icon: (
  ...
  {
    name: 'TikTok',
    href: brandProfiles.find((item) => item.name === 'TikTok')?.url ?? 'https://www.tiktok.com/@livana.official',
    icon: (
      <svg viewBox="0 0 24 24" className="h-5 w-5" aria-hidden="true" focusable="false">
        <path
          fill="currentColor"
  ...
  {
    name: 'Shopee',
    href: brandProfiles.find((item) => item.name === 'Shopee')?.url ?? 'https://shopee.vn/livana.official',
    icon: (
    ...
    <footer className="bg-[#0f3012] text-white">
      <div className="mx-auto grid max-w-6xl gap-8 px-4 py-12 md:grid-cols-3">
        ...
        <div className="space-y-3 text-sm text-white/80">
          ...
          <p className="text-xs leading-5 text-white/65">{brandSearchHint}</p>
